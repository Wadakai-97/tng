...
function InventoryManagement() {
    const [inventory, setInventory] = createSignal(sampleInventory);
    const [searchName, setSearchName] = createSignal('');
    const [searchCategory, setSearchCategory] = createSignal('');
    const [isSearchVisible, setIsSearchVisible] = createSignal(true);

    // フィルタリングされた在庫データを作成
    const filteredInventory = createMemo(() => {
        return inventory().filter(item =>
            item.product_name.includes(searchName()) &&
    ...
    return (
        <div class="inventory-list-container">
            ...
            <button onClick={toggleSearchVisibility} class="toggle-search-button">
                {isSearchVisible() ? '検索条件：非表示' : '検索条件：表示'}
            </button>
                ...
                <div class="search-container">
                    <div class="search-fields">
                        <div class="search-field">
                            ...
                            <input type="text" id="searchName" value={searchName()} onInput={(e) => setSearchName(e.target.value)} class="search-input" />
                        </div>
                        <div class="search-field">
                            <label for="searchCategory">カテゴリ</label>
                            <select id="searchCategory" value={searchCategory()} onInput={(e) => setSearchCategory(e.target.value)} class="search-select">
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="stock-update-container">
                                    <input type="number" min="0" value={item.stock_quantity} onChange={(e) => updateStockQuantity(item.product_id, parseInt(e.target.value))} class="stock-input" />
